...
#                 f"{text}\n\n"
#                 "Есть ли в тексте фобии (страхи, предубеждения, опасения и т.д.) перед искусственным интеллектом (ИИ)? Если они есть - в чем причина фобии?"
#             )
#         }
...
#         responses = pipe(messages, num_return_sequences=1)

#         # Обрабатываем каждый ответ
#         for response in responses:
#             try:
#                 # Проверяем, что response является словарем и содержит нужный ключ
#                 if isinstance(response, dict) and 'generated_text' in response:
#                     # Итерируем по результатам
#                     for text_result in response['generated_text']:
#                         if isinstance(text_result, dict) and 'content' in text_result:
#                             # Добавляем текст, убирая ненужные символы
#                             content = text_result['content'].replace('[/INST]\n', '').replace('\n', '').replace('[/INST]', '')
#                             print(333)
#                             print(content)
#                             print("===555===")
#                             llm_labels.append(content)  # Добавляем все результаты в llm_labels
#                         else:
#                             print("Unexpected generated text format:", text_result)
#                 else:
#                     print("Unexpected result format:", response)
#             except Exception as e:
...
# Проходим через массив текстов, обрабатывая их по одному
for text in tqdm(texts):
    # Создаем сообщение для вашего LLM с ожидаемым форматом ответа
    message = {
        "role": "system",
        "content": (
            f"{system_prompt}"
    ...
    # Используем torch.no_grad() для предотвращения вычисления градиентов
    with torch.no_grad():
        # Генерируем ответ для каждого текста
        response = pipe([message], num_return_sequences=1)

        try:
            # Проверяем, что response является списком и содержит нужный формат
            if isinstance(response, list) and len(response) > 0 and isinstance(response[0], dict) and 'generated_text' in response[0]:
                text_results = response[0]['generated_text']
                # Обрабатываем результат
                if isinstance(text_results, list) and len(text_results) > 0:
                    content = text_results[-1]['content']  # Получаем последний ответ от ассистента
                    llm_labels.append(content)  # Добавляем только содержимое
                else:
                    print("Unexpected text results format:", text_results)
            else:
                print("Unexpected result format:", response)
        except Exception as e:
            print("Error processing result:", e)

    # Проверяем длину текста и добавляем в llm_labels, если превышает 15000 символов
    if len(text) > 15000:
        llm_labels.append('Длинный текст')
        count += 1
