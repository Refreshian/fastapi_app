...
batch_size = 20
llm_labels = []
processed_texts = {}  # Словарь для хранения обработанных текстов и результатов
count = 0
                        ...
                        if isinstance(text_result, dict) and 'content' in text_result:
                            ...
                            for text in batch_texts:
                                if text not in processed_texts:
                                    processed_texts[text] = generated_content
                                    llm_labels.append(generated_content)
                                else:
                                    llm_labels.append(processed_texts[text])  # Если текст уже обработан, используем его результат
                        else:
...
for i, topic in enumerate(topic_model.get_topics().values()):  # Получаем ключевые слова тем
    ...
    print(response[0]['generated_text'][1]['content'].replace('[/INST]\n', '').replace('\n', '').replace('[/INST]', ''))
    print("+++555+++555+++")
