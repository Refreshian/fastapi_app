...
async def information_graph(index: int = None, 
    ...
    for key, val in fin_dict.items():
        ...
        values_data.append(author_dct)

    # Обработка времени. Приводим timeCreate в формат datetime.
    df_meta['timeCreate'] = pd.to_datetime(df_meta['timeCreate'], errors='coerce')
    df_meta.sort_values(by='timeCreate', inplace=True)
    df_meta.reset_index(drop=True, inplace=True)

    # Формируем диапазон бинов по 10 минут (600T)
    bins = pd.date_range(start=df_meta['timeCreate'].min(), end=df_meta['timeCreate'].max(), freq='600T')
    # Создаем дополнительный столбец 'bin' на основе pd.cut.
    df_meta['bin'] = pd.cut(df_meta['timeCreate'], bins=bins, right=False)
    # Преобразуем метки бинов в строку — оставляем начало интервала.
    df_meta['bin'] = df_meta['bin'].apply(lambda x: x.left if pd.notnull(x) else bins[-1])
    
    # Берем нужные интервальные данные
    df_bins = pd.DataFrame(bins, columns=['bin']).astype('datetime64[ns]').set_index('bin')
    df = df_bins.join(df_meta.set_index('bin'), how='left')
    # Для отсутствующих значений заполняем аудиторию = 0 и источник копируем из группы.
    df['audienceCount'] = df['audienceCount'].fillna(0).astype(int)
    df['hub'] = df['hub'].fillna(method='ffill')  # если источник отсутствует, можно попробовать заполнить предыдущим значением

    # Если в df присутствует строка без hub, оставляем её пустой – позже она будет проигнорирована при группировке.
    df.reset_index(inplace=True)
    df.rename(columns={'bin': 'timeCreate'}, inplace=True)
    df = df[['hub', 'timeCreate', 'audienceCount']]
    
    # Получаем список уникальных источников.
    unique_hubs = df['hub'].unique()

    # Для каждого источника гарантируем наличие строки по каждому бинy времени
    full_df = pd.DataFrame()
    for hub in unique_hubs:
        hub_df = df[df['hub'] == hub].copy()
        # Создаем DataFrame с полным списком бин для источника hub.
        hub_bins = pd.DataFrame({'timeCreate': bins})
        hub_bins['hub'] = hub
        hub_df = pd.merge(hub_bins, hub_df, on=['hub', 'timeCreate'], how='left')
        hub_df['audienceCount'] = hub_df['audienceCount'].fillna(0).astype(int)
        full_df = pd.concat([full_df, hub_df], ignore_index=True)
    
    # Исключаем пустые значения источника
    full_df = full_df[full_df['hub'] != '']
    
    # Группируем данные по источникам.
    hub_groups = full_df.groupby('hub')

    dynamicdata_audience = {}

    # Проходим по каждой группе (каждому источнику).
    for hub, hub_df in hub_groups:
        # Сортируем данные по времени (bin-область уже проставлена в столбец timeCreate).
        hub_df = hub_df.sort_values(by='timeCreate')
        
        cumulative_audience = 0
        bin_dict = {}
        
        # Проходим по всем строкам для источника.
        for _, row in hub_df.iterrows():
            # Получаем метку времени начала бин‑интервала в формате unixtime (секунды).
            bin_ts = int(row["timeCreate"].timestamp())
            cumulative_audience += row["audienceCount"]
            # Записываем накопленное значение аудитории для данного бин‑интервала.
            bin_dict[bin_ts] = cumulative_audience
            
        dynamicdata_audience[hub] = bin_dict


    print(f"Количество сообщений: {num_messages}")

    def count_unique_authors(data_list):
        authors = set()
    ...
    num_unique_authors = count_unique_authors(values_data)
    print(f"Количество уникальных авторов: {num_unique_authors}")

    print(555777, flush=True)
    print(full_df[:5].values, flush=True)

    # Приводим возраст к строке для автора и репостов
    for author_data in values_data:
        author_data['author']['age'] = str(author_data['author']['age'])
        for repost_data in author_data['reposts']:
            repost_data['age'] = str(repost_data['age'])

    return dynamicdata_audience
...
async def get_text_clusters(user_id: int,
                            folder_name: str,
                            file_name: str,
                            session: AsyncSession = Depends(get_db),
                            threshold: float = 0.8):
    # Проверка корректности входного параметра user_id
    if user_id < 1:
        raise HTTPException(status_code=400, detail="user_id must be a positive integer.")

    # Получаем исходные тексты для кластеризации
    ...
    # Создаём DataFrame с текстовыми данными в колонке 'Ответ модели'
    df = pd.DataFrame({"Ответ модели": texts})

    # Векторизация текстов с использованием TF-IDF
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(df["Ответ модели"])

    # Вычисление косинусного сходства и преобразование его в матрицу расстояний
    cos_sim = cosine_similarity(tfidf_matrix, dense_output=False)
    cos_dist = 1 - cos_sim
    cos_dist = np.clip(cos_dist, 0, None)
    if np.any(cos_dist < 0):
        raise ValueError("Найдены отрицательные значения в матрице расстояний.")

    # Кластеризация текстов с использованием DBSCAN (metric='precomputed')
    dbscan = DBSCAN(metric="precomputed", eps=threshold, min_samples=2)
    labels = dbscan.fit_predict(cos_dist)

    # Формирование списка результатов в виде кортежей: (номер кластера, текст)
    results = []
    for idx, label in enumerate(labels):
        results.append((label, df["Ответ модели"].iloc[idx]))

    # Получение данных пользователя из Redis, декодирование ключей и значений
    ...
    # Поиск HTML-файла среди файлов пользователя
    ...
    for file_info in html_files:
        if file_info.get("html-file") == file_name:
            ...
            html_file_path = os.path.join(
                "/home/dev/fastapi/analytics_app/data",
                str(user_id),
    ...
    # Загрузка словаря с темами для ElasticSearch
    file_path = "/home/dev/fastapi/analytics_app/data/indexes.pkl"
    indexes = load_dict_from_pickle(file_path)

    # Если строка запроса отсутствует, устанавливаем значение 'all'
    if info_html.get("query_str") is None:
        info_html["query_str"] = "all"

    # Выполнение запроса к ElasticSearch для получения метаданных
    data = elastic_query(
        theme_index=indexes[info_html["index_number"]],
        query_str=info_html["query_str"],
        min_date=info_html.get("min_date"),
        max_date=info_html.get("max_date"),
    )
    data = pd.DataFrame(data)

    # Объединение LLM с метаданными:
    # Переименовываем столбец url -> text_url для последующего объединения
    data.rename(columns={"url": "text_url"}, inplace=True)
    if "authorObject" in data.columns:
        try:
            authors_df = pd.DataFrame(list(data["authorObject"].values))
            data = data.join(authors_df)
            data.rename(columns={"url": "author_url"}, inplace=True)
        except Exception as ex:
            print("Ошибка при объединении с authorObject:", ex)

    # Оставляем только необходимые колонки
    cols = [
        "timeCreate", "hub", "author_url", "fullname", "text_url", "author_type",
        "sex", "age", "hubtype", "commentsCount", "audienceCount", "repostsCount",
        "likesCount", "er", "viewsCount", "toneMark", "country", "region",
    ]
    data = data[cols]

    # Объединяем результаты кластеризации и данные из ElasticSearch.
    clusters_df = pd.DataFrame(results, columns=["Кластер", "Тематика текста"])
    df_join = clusters_df.join(data, how="inner", lsuffix="_df1", rsuffix="_df2")
    df_join.columns = ["Кластер", "Тематика текста"] + cols

    # Сохранение эмбеддингов в PostgreSQL для экономии оперативной памяти.
    # В данном примере filename формируется с префиксом embeddings_
    new_filename = f"embeddings_{file_name}"
    ...
    # После сохранения можно удалить вектора из памяти
    del tfidf_matrix

    # Возвращаем результат, приводя значения NaN к None
    return {
...
if __name__ == "__5001__":
    uvicorn.run("5001:app", host="0.0.0.0", port=5001, reload=True)
